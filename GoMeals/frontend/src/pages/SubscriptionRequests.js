...
function SubscriptionRequests() {
    ...
    useEffect(() => {
        setIsLoading(true);
        axios
            .get(`http://localhost:8080/subscription/get/pending/${loggedInUser.supId}`)
            .then((response) => {
                                ...
                                {subscriptionRequestList.map((subRequest, index) => {
                                    return (
                                        <tr key={subRequest.sub_id}>
                                            ...
                                            <td>{subRequest.customer?.cust_fname + " " + subRequest.customer?.cust_lname}</td>
                                            <td>{subRequest.sub_date}</td>
                                            <td>
                                                <Button variant="success" value="Approved" className="me-2 py-1" onClick={(event) => handleRequest(event, index)} >Approve</Button>
                                                <Button variant="danger" value="Rejected" className="me-2 py-1" onClick={(event) => handleRequest(event, index)} >Reject</Button></td>
                                        </tr>
                                    );
                                })
                                }
